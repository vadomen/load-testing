...
export const testAlligatorService = (payload: SetupPayloadModel) => {
  ...
  checkResponse(myAlligators, 'Test list alligators');

  const myAlligatorsCollection = parseBody(myAlligators);
  if (!myAlligatorsCollection.length) exec.test.abort('Tails: Alligators collection is empty for the user');

  const alligatorId = myAlligatorsCollection[0].id;
  checkResponse(getAnAlligator(serviceAlligatorBaseUrl, alligatorId, headers), `Check alligator ${alligatorId}`);
};
